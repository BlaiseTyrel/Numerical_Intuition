...
class NormalPlay:
    ...
    def run(self):
        ...
        self.orangeplay_button, self.orangelevel_button, self.back_button = load_buttons("orangeplay", 'orangelevel', 'back')

        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN:
                pos = pygame.mouse.get_pos()
                if self.orangeplay_button.is_clicked(pos):
                    self.gamestatemanager.set_state('normalgame')
                elif self.back_button.is_clicked(pos):
                    self.gamestatemanager.set_state('start')
                elif self.orangelevel_button.is_clicked(pos):
                    self.gamestatemanager.set_state('level')

        self.orangeplay_button.draw(self.display)
        self.orangelevel_button.draw(self.display)
